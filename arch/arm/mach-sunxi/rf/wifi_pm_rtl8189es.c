...
static int rtl8189es_wl_regon = 0;
static char *axp_name[4] = {NULL};

static int sunxi_rtl8189es_gpio_req(struct gpio_config *gpio)
{
    int ret = 0;
    char pin_name[8] = {0};
    ...
    if (ret) {
        rtl8189es_msg("set gpio %s mulsel failed.\n",pin_name);
        return -1;
    }
    ...
    if (gpio->pull != GPIO_PULL_DEFAULT){
        ...
        ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
        if (ret) {
            rtl8189es_msg("set gpio %s pull mode failed.\n",pin_name);
            return -1;
        }
    }
    ...
    if (gpio->drv_level != GPIO_DRVLVL_DEFAULT){
        ...
        ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
        if (ret) {
            rtl8189es_msg("set gpio %s driver level failed.\n",pin_name);
            return -1;
        }
    }
    ...
    if (gpio->data != GPIO_DATA_DEFAULT) {
        ...
        ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
        if (ret) {
            rtl8189es_msg("set gpio %s initial val failed.\n",pin_name);
            return -1;
        }
    }
...
static int rtl8189es_gpio_ctrl(char* name, int level)
{
    int i = 0;
    int gpio = 0;
    char * gpio_name[2] = {"rtl8189es_shdn", "rtl8189es_wl_regon"};
        ...
        if (strcmp(name, gpio_name[i]) == 0) {
            switch (i)
            {
                case 0: /*rtl8189es_shdn*/
                    gpio = rtl8189es_shdn;
                    break;
                case 1: /*rtl8189es_wl_regon*/
                    gpio = rtl8189es_wl_regon;
                    break;
                default:
...
static void rtl8189es_power(int mode, int *updown)
{
    if (mode) {
        if (*updown) {
            rtl8189es_gpio_ctrl("rtl8189es_wl_regon", 1);
            mdelay(100);
        } else {
            rtl8189es_gpio_ctrl("rtl8189es_wl_regon", 0);
            mdelay(100);
        }
...
void rtl8189es_gpio_init(void)
{
    ...
    sunxi_rtl8189es_gpio_req(&val.gpio);

    type = script_get_item(wifi_para, "rtl8189es_wl_regon", &val);
    if (SCIRPT_ITEM_VALUE_TYPE_PIO!=type)
        rtl8189es_msg("get rtl8189es rtl8189es_wl_regon gpio failed\n");
    else
        rtl8189es_wl_regon = val.gpio.gpio;
    sunxi_rtl8189es_gpio_req(&val.gpio);
